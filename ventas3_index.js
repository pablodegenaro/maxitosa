...
function listarDocumentos(search = "") {
    ...
    tabla_documentos = $('#documentos_data').dataTable({
        ...
        "ajax":
        {
            url: 'ventas3_index_data.php?op=listar_docs',
            type: "POST",
            dataType: "json",
...
function listarDocumentosBorrar(search = "") {
    ...
    tabla_documentos_borrar = $('#documentos1_data').dataTable({
        ...
        "ajax":
        {
            url: 'ventas3_index_data.php?op=listar_docs_borrar',
            type: "POST",
            dataType: "json",
            data: { search: search, tipo: tipo_input },
            error: function (e) {
...
function buscarDoc() {
    ...
    $.ajax({
        url: "ventas3_index_data.php?op=buscar_doc",
        type: "POST",
        dataType: "json",
        data: {
...
$(document).on("click", "#btn_modal_imp_aceptar", function () {
    ...
    if (numerod_input !== "" && tipo_input !== "" && nrounico_input !== "") {
        switch (tipo_input) {
        case 'A': window.open('ventas3_fac_pdf1.php?&i=' + nrounico_input, '_blank'); break;
        case 'B': window.open('ventas3_devolfac_pdf.php?&i=' + nrounico_input, '_blank'); break;
        case 'C': window.open('ventas3_ne_pdf.php?&i=' + nrounico_input, '_blank'); break;
        case 'D': window.open('ventas3_devolne_pdf.php?&i=' + nrounico_input, '_blank'); break;
        case 'E': window.open('ventas3_ped_pdf.php?&i=' + nrounico_input, '_blank'); break;
        case 'F': window.open('ventas3_presu_pdf.php?&i=' + nrounico_input, '_blank'); break;
        }
...
$(document).on("click", "#btn_modal_borrar_aceptar", function () {
    ...
    if (numerod_input !== "" && tipo_input !== "" && nrounico_input !== "") {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: "ventas3_index_data.php?op=eliminar_doc",
                    method: "POST",
                    dataType: "json",
                    data: {
                        numerod: numerod_input,
