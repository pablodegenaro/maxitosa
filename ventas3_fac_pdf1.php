...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$querySaconf = mssql_query("SELECT TOP 1 Texto2 FROM SACONF WHERE CodSucu='$sucursal'");
		...
		$arr_licencia = array();
		$texto2 = mssql_result($querySaconf,0,"Texto2");
		$arr_texto2 = explode(" ", $texto2);
		foreach ($arr_texto2 as $text) {
			if (strlen($arr_licencia[$pos]) < $lenghtCaracters[$pos]) {
				$arr_licencia[$pos] .= " $text";
			} else {
				$pos++;
				$arr_licencia[$pos] .= $text;
			}
		...
		$this->Cell(71);
		$this->SetFont('Arial','',8);
		$this->Cell(28,3, /* utf8_decode("Licencia de Licor N°") */ utf8_decode((array_key_exists(0, $arr_licencia)) ? $arr_licencia[0] : ""),'',1,'L');
		//$this->Ln(1);
		$this->SetFont('Arial','',8);
		...
		$this->Cell(80);
		$this->SetFont('Arial','',8);
		$this->Cell(28,3, /* utf8_decode("MY-315-MN-495 REGION") */ utf8_decode((array_key_exists(1, $arr_licencia)) ? $arr_licencia[1] : ""),'',1,'L');
		$this->SetFont('Arial','',8);
		...
		$this->Cell(114);
		$this->SetFont('Arial','',8);
		$this->Cell(28,3, /* utf8_decode("GUAYANA ESTADO BOLIVAR") */ utf8_decode((array_key_exists(2, $arr_licencia)) ? $arr_licencia[2] : ""),'',1,'L');
		$this->Cell(200);
		$this->SetFont('Arial','',8);
		$this->Cell(28,3, /* utf8_decode("PATENTE AE NRO. 17650 RUC:") */ utf8_decode((array_key_exists(3, $arr_licencia)) ? $arr_licencia[3] : ""),'',1,'L');
...
while ($i = mssql_fetch_assoc($saitemfac)) {
	...
	$pdf->Cell(19,4, number_format($i['pvp'], 2, ',', '.'),'',0,'R',0);
