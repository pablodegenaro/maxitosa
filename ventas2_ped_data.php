        ...
        if ($_SESSION['codsucu']) 
        {
            ...
            if ($_SESSION['codesta']) 
            {
                ...
                if ($flag_exis) {
                    ...
                    $querylengh = mssql_query("SELECT ValueInt FROM SACORRELSIS WHERE FieldName='LenCorrel' AND CodSucu='$codsucu'");
                    $lengh = (mssql_num_rows($querylengh)>0) ? mssql_result($querylengh, 0,"ValueInt") : 8;
                    $query = mssql_query("SELECT FieldName, Prefijo, ValueInt FROM SACORRELSIS WHERE FieldName='PrxPedido' AND CodSucu='$codsucu'");
                    ...
                    if ($correl_nuevo != "") {
                        ...
                        $query = mssql_query("UPDATE SACORRELSIS SET ValueInt=ValueInt+1 WHERE FieldName='PrxPedido' AND CodSucu='$codsucu'");
                        ...
                        foreach ($arr_prod as $i => $codprod) {
                            if (!empty($codprod)) {
                                ...
                                $proc_fact = mssql_query("EXEC [App_Ped_Presu_Items] @NumeroD ='$correl_nuevo', @TipoFac ='$tipofac_facturar', @CodSucu ='$codsucu', @NroOrg ='$numerod_c', @TipoOrg ='$tipofac_c', @Deposito = '$codubic', @Vendedor='$codvend', @NroLinea ='$nroLinea', @CodItem ='$codItem', @Cantidad ='$cantidad', @EsUnida ='$esunid', @TipoPvp = '$tipopvp', @Factor ='$tasa', @FechaEmi ='$fechaemi', @DiasVen ='0' ")
                        ...
                        if ($flag_items) {
                            ...
                            $procesar = mssql_query("EXEC [App_Ped_Presu_Header] @NumeroD ='$correl_nuevo', @TipoFac ='$tipofac_facturar', @CodClie ='$codclie', @CodSucu ='$codsucu', @CodEsta ='$codesta', @CodUsua ='$user', @TipoOrg ='$tipofac_c', @NroOrg ='$numerod_c', @Deposito = '$codubic', @Vendedor='$codvend', @Factor ='$tasa', @Anticipo ='$monto_anticipo', @TipoOpe ='$tipo_ope', @FechaEmi ='$fechaemi', @DescuentoUno = '$porcentaje_primer_des', @Notas1 = '$coment1', @Notas2 = '$coment2', @Notas3 = '$coment3', @Notas4 = '$coment4', @Notas5 = '$coment5' ")
                            or ($mensaje_err.=('MSSQL Error: ' . mssql_get_last_message()."<br/>"));
                        }
                    }
