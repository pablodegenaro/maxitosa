...
$datos = Mssql::fetch_assoc(
    mssql_query("SELECT NumeroD, CodItem, Descrip1, CodUbic, Cantidad, Precio/factorp as Precio, (TotalItem+MtoTax)/factorp AS Total 
                        FROM saitemfac WHERE TipoFac IN ('A','C') AND numerod = '$numerod'")
);
...
foreach ($datos as $key => $row) {
    ...
    $sub_array[] = Functions::rdecimal($row["Precio"],2);
