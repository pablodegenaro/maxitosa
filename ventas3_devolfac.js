...
function correlYtasa() {
    $.post("ventas3_devolfac_data.php?op=index", function (data, status) {
...
$(document).ready(function () {
    $("#clie").change(() => {
        if ($("#clie").val() !== "") {
            ...
            $.post("ventas3_devolfac_data.php?op=datos_clie", { codclie: codclie }, function (data, status) {
                if (!jQuery.isEmptyObject(data)) {
...
function cantPrdResult(search = "") {
    ...
    $.ajax({
        ...
        url: "ventas3_devolfac_data.php?op=buscar_cant_prd",
...
function mostrarCXC() {
    ...
    if (carga_nc == 0) {
        ...
        $.ajax({
            async: false,
            url: "ventas3_devolfac_data.php?op=listar_cxc",
            method: "POST",
            dataType: "json",
...
function listarProductos(search = "") {
    ...
    tabla_productos = $('#productos_data').dataTable({
        ...
        "ajax":
        {
            url: 'ventas3_devolfac_data.php?op=listar_prd',
            type: "POST",
            dataType: "json",
            data: {
                search: search,
...
function listarDocumentos(search = "") {
    ...
    tabla_documentos = $('#documentos_data').dataTable({
        ...
        "ajax":
        {
            url: 'ventas3_devolfac_data.php?op=listar_docs',
            type: "POST",
            dataType: "json",
...
function seleccionarPrd(codprod, idx = "") {
    ...
    if (renglones <= cant_renglones) {
        ...
        $.post("ventas3_devolfac_data.php?op=datos_prod",
        {
            codprod: codprod,
            cant: cant,
            unid: unid,
...
function seleccionarDoc(numerod, tipofac) {
    $.ajax({
        url: "ventas3_devolfac_data.php?op=datos_doc",
        method: "POST",
        dataType: "json",
        data: {
            numerod: numerod,
...
$.ajax({
    url: "ventas3_devolfac_data.php?op=notacredito",
    type: "POST",
    data: formData,
    dataType: "json",
    contentType: false,
