...
header("Content-Disposition: attachment; filename=resumen_cobranza_rango.xls");
...
for ($i=0; $i < mssql_num_rows($query); $i++) {
	...
	if (($tipofac == 41)  ) {
		if($dias <= 7){
			...
			$more[$o]['fechat'] = mssql_result($query, $i, 'fechafac');
		...
		<div class="col-12">
			<div class="card card-saint">
				...
				<div class="card-body">
					...
					<?php echo "TOTAL DOCUMENTOS: ".count($array)." TOTAL MONTO: ".number_format($suma, 2, '.', ','); ?>
					<?php 
					echo "<script> prehide(); </script>";
				?>