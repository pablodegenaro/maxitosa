...
function correlYtasa() {
    $.post("ventas2_presu_data.php?op=index", function (data, status) {
...
$(document).ready(function () {
    $("#clie").change(() => {
        if ($("#clie").val() !== "") {
            ...
            $.post("ventas2_presu_data.php?op=datos_clie", { codclie: codclie }, function (data, status) {
                if (!jQuery.isEmptyObject(data)) {
...
function cantPrdResult(search = "") {
    ...
    $.ajax({
        ...
        url: "ventas2_presu_data.php?op=buscar_cant_prd",
...
function listarProductos(search = "") {
    ...
    tabla_productos = $('#productos_data').dataTable({
        ...
        "ajax":
        {
            url: 'ventas2_presu_data.php?op=listar_prd',
            type: "POST",
            dataType: "json",
            data: {
                search: search,
...
function seleccionarPrd(codprod, idx = "") {
    ...
    if (renglones <= cant_renglones) {
        ...
        $.post("ventas2_presu_data.php?op=datos_prod",
        {
            codprod: codprod,
            cant: cant,
            unid: unid,
...
function procesarPresupuestoCr(e) {
    ...
    $.ajax({
        url: "ventas2_presu_data.php?op=presupuestar",
        type: "POST",
        data: formData,
        dataType: "json",
        contentType: false,
