...
if ($_SESSION['login']) {
    ...
    $correl1 = mssql_query("SELECT max(correl) as correl from appfacturasft ");
    if ( mssql_result($correl1, 0, 'correl') >= 1) {
        $correl =  mssql_result($correl1, 0, 'correl') + 1; 
    } else {
        $correl = 1 ; 
    };
    $proce_cabezera = mssql_query("EXEC [Envio_cabezeraft] @fechae ='$fechae', @usuario ='$usuario', @correl =$correl, @fechad ='$fechad',   @nota ='$nota', @cedula_chofer ='$cedula_chofer', @placa ='$placa' ");

    
    $checked_contador = count($producto);
    foreach($producto as $seleccion) {
        $proce_detalle    = mssql_query("EXEC Envioft @NroPhP = '$seleccion' ");
    }; 
...
// Sender &amp; Recipient
    ...
    //$mail->AddCC("");
    //$mail->AddBCC("");
    $mail->Subject = "Se Creo el Despacho # $correl FT";
    $body="El usuario $usuario Creo el Despacho # $correl . FT";
    $mail->Body=$body;
    $mail->Send();


    echo("<script>location.href = 'principal1.php?page=despacho_visualft&mod=1&correl2=$correl&usuario=$usuario';</script>");
