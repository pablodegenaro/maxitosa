...
switch ($_GET["op"]) {
    ...
    $datos =  Mssql::fetch_assoc(
        mssql_query("SELECT p.CodProd, p.Descrip, CONVERT(INT, p.CantEmpaq) CantEmpaq, p.Unidad, p.UndEmpaq, p.Precio$precio/(CASE WHEN  '$unid' = '1' THEN p.CantEmpaq ELSE 1 END) AS Precio, 
        ...
        if ($_SESSION['codsucu']) 
        {
            ...
            if ($_SESSION['codesta']) 
            {
                ...
                if ($flag_exis) {
                    ...
                    $querylengh = mssql_query("SELECT ValueInt FROM SACORRELSIS WHERE FieldName='LenCorrel' AND CodSucu='$codsucu'");
                    $lengh = (mssql_num_rows($querylengh)>0) ? mssql_result($querylengh, 0,"ValueInt") : 8;
                    $query = mssql_query("SELECT FieldName, Prefijo, ValueInt FROM SACORRELSIS WHERE FieldName='PrxNotaCr' AND CodSucu='$codsucu'");
                    ...
                    if ($correl_nuevo != "") {
                        ...
                        $query = mssql_query("UPDATE SACORRELSIS SET ValueInt=ValueInt+1 WHERE FieldName='PrxNotaCr' AND CodSucu='$codsucu'");
                            ...
                            if ($procesar && $numerod_c != '' && $emi_nc == 1) {
                                if (isset($arr_unico) && count($arr_unico)>0) {
                                    foreach ($arr_unico as $index => $nrounico) {
                                        ...
                                        mssql_query("EXEC [App_Fac_Ne_Devolucion] @Monto ='$monto_pago', @NroUnico='$nrounico', @NumeroD ='$numerod_cxc', @CodSucu ='$codsucu', @CodClie ='$codclie', @FechaEmi ='$fechaemi' ");
                                    }
                                }
                            }
                        }
