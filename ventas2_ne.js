...
function correlYtasa() {
    $.post("ventas2_ne_data.php?op=index", function (data, status) {
...
$(document).ready(function () {
    $("#clie").change(() => {
        if ($("#clie").val() !== "") {
            ...
            $.post("ventas2_ne_data.php?op=datos_clie", { codclie: codclie }, function (data, status) {
                if (!jQuery.isEmptyObject(data)) {
...
function cantPrdResult(search = "") {
    ...
    $.ajax({
        ...
        url: "ventas2_ne_data.php?op=buscar_cant_prd",
...
function actualizarExistenciasEnCasoError() {
    ...
    $.ajax({
        url: "ventas2_ne_data.php?op=existencias",
        type: "POST",
        data: formData,
        dataType: "json",
        contentType: false,
...
function listarProductos(search = "") {
    ...
    tabla_productos = $('#productos_data').dataTable({
        ...
        "ajax":
        {
            url: 'ventas2_ne_data.php?op=listar_prd',
            type: "POST",
            dataType: "json",
            data: {
                search: search,
...
function listarDocumentos(search = "") {
    ...
    tabla_documentos = $('#documentos_data').dataTable({
        ...
        "ajax":
        {
            url: 'ventas2_ne_data.php?op=listar_docs',
            type: "POST",
            dataType: "json",
...
function seleccionarPrd(codprod, idx = "") {
    ...
    if (renglones <= cant_renglones) {
        ...
        $.ajax({
            async: false,
            cache: false,
            url: "ventas2_ne_data.php?op=datos_prod",
            method: "POST",
            dataType: "json",
            data: {
                codprod: codprod,
...
function seleccionarDoc(numerod, tipofac) {
    $.ajax({
        async: false,
        cache: false,
        url: "ventas2_ne_data.php?op=datos_doc",
        method: "POST",
        dataType: "json",
        data: {
            numerod: numerod,
...
function setFechaV(tipo = '') {
    ...
    function procesarNotaEntrega(e) {
    e.preventDefault(); //No se activará la acción predeterminada del evento
    ...
    $.ajax({
        url: "ventas2_ne_data.php?op=generarnota",
        type: "POST",
        data: formData,
        dataType: "json",
        contentType: false,
    ...
    if (icono.includes('error')) {
        ...
        arr_notas = Array();
                    //setTimeout(() => {  window.open('ventas2_ne_pdf.php?&i=' + data.id, '_blank'); }, 2000);
    }
}
});
}
