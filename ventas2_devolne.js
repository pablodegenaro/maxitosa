...
function correlYtasa() {
    $.post("ventas2_devolne_data.php?op=index", function (data, status) {
...
$(document).ready(function () {
    $("#clie").change(() => {
        if ($("#clie").val() !== "") {
            ...
            $.post("ventas2_devolne_data.php?op=datos_clie", { codclie: codclie }, function (data, status) {
                if (!jQuery.isEmptyObject(data)) {
...
function cantPrdResult(search = "") {
    ...
    $.ajax({
        ...
        url: "ventas2_devolne_data.php?op=buscar_cant_prd",
...
function listarProductos(search = "") {
    ...
    tabla_productos = $('#productos_data').dataTable({
        ...
        "ajax":
        {
            url: 'ventas2_devolne_data.php?op=listar_prd',
            type: "POST",
            dataType: "json",
            data: {
                search: search,
...
function listarDocumentos(search = "") {
    ...
    tabla_documentos = $('#documentos_data').dataTable({
        ...
        "ajax":
        {
            url: 'ventas2_devolne_data.php?op=listar_docs',
            type: "POST",
            dataType: "json",
...
function seleccionarPrd(codprod, idx = "") {
    ...
    if (renglones <= cant_renglones) {
        ...
        $.post("ventas2_devolne_data.php?op=datos_prod",
        {
            codprod: codprod,
            cant: cant,
            unid: unid,
...
function seleccionarDoc(numerod, tipofac) {
    $.ajax({
        url: "ventas2_devolne_data.php?op=datos_doc",
        method: "POST",
        dataType: "json",
        data: {
            numerod: numerod,
...
$.ajax({
    url: "ventas2_devolne_data.php?op=devolne",
    type: "POST",
    data: formData,
    dataType: "json",
    contentType: false,
...
if (icono.includes('error')) {
    ...
    $("#btn_limpiar").trigger('click');
                //setTimeout(() => {  window.open('ventas2_devolne_pdf.php?&i=' + data.id, '_blank'); }, 2000);
}
}
});
