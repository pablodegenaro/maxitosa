...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->Cell(18);
		$this->SetFont('Arial','',8);
		$this->Cell(10,5, "Zona:",'',0,'L');
		$this->SetFont('Arial','B',8);
		$this->Cell(21,5, mssql_result($querySafact,0,"zona"),'',0,'L');
		$this->Cell(116);
		$this->SetFont('Arial','',8);
		...
		$this->Cell(26,5.5,'','B');
		$this->Cell(18,5.5,'Percibido','B',0,'C',0);
		$this->Cell(6,5.5,'','B');
...
$saitemfac = mssql_query("SELECT item.CodItem, Descrip1, item.Precio, TotalItem, item.MtoTax, Descto, item.MtoTax+TotalItem Total, grado_alcoholico, capacidad_botella,
...
while ($i = mssql_fetch_assoc($saitemfac)) {
	...
	$pdf->SetFont('Arial','',8);
	$pdf->Cell(14,4, number_format($i['Precio'], 2, ',', '.'),'',0,'R',0);
	$pdf->Cell(14,4, number_format($i['Descto'], 2, ',', '.'),'',0,'R',0);
	$pdf->Cell(23,4, number_format($i['TotalItem'], 2, ',', '.'),'',0,'R',0);
	$pdf->Cell(23,4, number_format($i['porc_iva'], 0, ',', '.').' %','',0,'R',0);
	$pdf->Cell(21,4, number_format($i['pvp'], 2, ',', '.'),'',0,'R',0);
	$pdf->Cell(19,4, number_format($i['ial'], 2, ',', '.'),'',0,'R',0);
	$pdf->Cell(20,4, number_format($i['TotalItem'] + $i['pvp'] + $i['ial'], 2, ',', '.'),'',1,'R',0);
...
$pdf->Cell(205); $pdf->Cell(35,4,'IVA PERCIBIDO:','',0,'L',0);
