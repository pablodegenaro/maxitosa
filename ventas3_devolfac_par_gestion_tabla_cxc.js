...
function calculos_CXC() {
    ...
    let temp = (montopago + monto_anticipo);
    $('#btn_accept_nc').prop('disabled', 
        !(
            (montototalnc.format_money(2, 3, '.', ',') == (temp+0.01).format_money(2, 3, '.', ',')) ||
            (montototalnc.format_money(2, 3, '.', ',') == (temp-0.01).format_money(2, 3, '.', ',')) ||
            (montototalnc.format_money(2, 3, '.', ',') == (temp).format_money(2, 3, '.', ','))
        )
    );
}