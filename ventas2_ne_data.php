...
switch ($_GET["op"]) {
    ...
    $sacorrel =  mssql_query("SELECT Prefijo, ValueInt FROM SACORRELSIS WHERE FieldName='PrxNotaEn' AND CodSucu='$codsucu'");
    ...
    $datos =  Mssql::fetch_assoc(
        mssql_query("SELECT p.CodProd, p.Descrip, CONVERT(INT, p.CantEmpaq) CantEmpaq, CONVERT(INT, e.Existen) Cajas, CONVERT(INT, e.ExUnidad) Botellas,  p.Unidad, p.UndEmpaq, ISNULL(CONVERT(INT, e.CantCom),0) CajasCompr, ISNULL(CONVERT(INT, e.UnidCom),0) BotellasCompr,
            p.Precio$precio/(CASE WHEN  '$unid' = '1' THEN p.CantEmpaq ELSE 1 END) AS Precio, p.EsExento,
    ...
    $data = array (
        ...
        "exento" => $datos['EsExento'],
        "ial" => $datos['ial'],
        "pvp" => $datos['pvp'],
        "iva" => $datos['iva'],
        "ial_d" => $datos['ial']/$tasa,
        "pvp_d" => $datos['pvp']/$tasa,
        "iva_d" => $datos['iva']/$tasa,
        "precio"  => $datos["Precio"],
    ...
    switch (true) {
        ...
        $datos = Mssql::fetch_assoc(
            mssql_query("SELECT NumeroD, Descrip, TipoFac, MtoTotal, FechaE, ISNULL(MtoTotal/FactorP, 0) Mtotald 
                FROM SAFACT WHERE TipoFac IN ('E','F') AND CodSucu='$sucursal' AND (NumeroD LIKE '%$search%' OR Descrip LIKE '%$search%')")
        );
        break;
        case ($search=='' && $codclie==''):
        $datos = Mssql::fetch_assoc(
            mssql_query("SELECT NumeroD, Descrip, TipoFac, MtoTotal, FechaE, ISNULL(MtoTotal/FactorP, 0) Mtotald 
                FROM SAFACT WHERE TipoFac IN ('E','F') AND CodSucu='$sucursal'")
        );
        break;
        case ($search!=='' && $codclie!==''):
        $datos = Mssql::fetch_assoc(
        ...
        $datos = Mssql::fetch_assoc(
            mssql_query("SELECT NumeroD, Descrip, TipoFac, MtoTotal, FechaE, ISNULL(MtoTotal/FactorP, 0) Mtotald 
                FROM SAFACT WHERE TipoFac IN ('E','F') AND CodSucu='$sucursal' AND CodClie='$codclie'")
        );
        break;
    }
    ...
    $safact =  Mssql::fetch_assoc(
        mssql_query("SELECT NumeroD, f.Descrip, TipoFac, f.CodClie, v.CodVend, v.Descrip NomperVend, c.TipoPVP, 
            ...
            convert(decimal(18,2), COALESCE(((Descto1*100)/NULLIF(Monto,0)), 0)) PorcDesc,
    ...
    $saitemfac =  Mssql::fetch_assoc(
        mssql_query("SELECT CodItem, Descrip1, item.CodUbic, Cantidad, item.EsUnid, p.UndEmpaq, CONVERT(INT, p.CantEmpaq) CantEmpaq, Precio, Precio/item.FACTORP AS Preciod, item.FactorP, TipoPVP, Descto, Descto/item.FactorP AS Desctod, 
            CONVERT(INT, e.Existen) Cajas, CONVERT(INT, e.ExUnidad) Botellas, ISNULL(CONVERT(INT, e.CantCom),0) CajasCompr, ISNULL(CONVERT(INT, e.UnidCom),0) BotellasCompr, p.EsExento,
        ...
        if ($_SESSION['codsucu']) 
        {
            ...
            if ($_SESSION['codesta']) 
            {
                    ...
                    if ($codprod!='') {
                        ...
                        if (mssql_num_rows($query) > 0) {
                            ...
                            if ($esunid==1 && $cantidad > $cantEmpaq) {
                                ...
                                if ($tipofac_c=='E') {
                                    if ( ($esunid==0 && $cantidad > ($bul)) || ($esunid==1 && ( $cantidad > ((($bul)>0) ? ((($bul)*$cantEmpaq) + ($paq)) : ($paq))  ) )) {
                                        $flag_exis = false;
                ...
                if ($flag_exis) {
                    ...
                    $querylengh = mssql_query("SELECT ValueInt FROM SACORRELSIS WHERE FieldName='LenCorrel' AND CodSucu='$codsucu'");
                    $lengh = (mssql_num_rows($querylengh)>0) ? mssql_result($querylengh, 0,"ValueInt") : 8;
                    $query = mssql_query("SELECT FieldName, Prefijo, ValueInt FROM SACORRELSIS WHERE FieldName='PrxNotaEn' AND CodSucu='$codsucu'");
                    ...
                    if ($correl_nuevo != "") {
                        ...
                        $query = mssql_query("UPDATE SACORRELSIS SET ValueInt=ValueInt+1 WHERE FieldName='PrxNotaEn' AND CodSucu='$codsucu'");
                        ...
                        foreach ($arr_prod as $i => $codprod) {
                            if (!empty($codprod)) {
                                ...
                                $proc_fact = mssql_query("EXEC [App_Fac_Ne_Items] @NumeroD ='$correl_nuevo', @TipoFac ='$tipofac_facturar', @CodSucu ='$codsucu', @NroOrg ='$numerod_c', @TipoOrg ='$tipofac_c', @Deposito = '$codubic', @Vendedor='$codvend', @NroLinea ='$nroLinea', @CodItem ='$codItem', @Cantidad ='$cantidad', @EsUnida ='$esunid', @TipoPvp = '$tipopvp', @Factor ='$tasa', @FechaEmi ='$fechaemi', @DiasVen ='$diasven' ")
                    ...
                    if($procesar){
                        ...
                        $output = array(
                            ...
                            "title" => 'Completado',
                            "mensaje" => "Nota de Entrega generada exitosamente!",
                            "icono"   => "success"
                        );
                    } else {
                        $output = array(
                            "id"   => "",
                            "title" => 'Atención!',
                            "mensaje" => "Ocurrió un error al Generar N/E! ".$mensaje_err,
                            "icono"   => "error"
                        );
                    }
