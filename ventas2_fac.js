...
function correlYtasa() {
    $.post("ventas2_fac_data.php?op=index", function (data, status) {
...
$(document).ready(function () {
    $("#clie").change(() => {
        if ($("#clie").val() !== "") {
            ...
            $.post("ventas2_fac_data.php?op=datos_clie", { codclie: codclie }, function (data, status) {
                if (!jQuery.isEmptyObject(data)) {
    ...
    $("#primer_des").on('keyup', () => {
        let value = parseFloat($("#primer_des").val());
        if (value < 0) {
            $("#primer_des").val(0);
        } else if (value > 100) {
            $("#primer_des").val(100);
    ...
    $("#anticipo").on('keyup', () => {
        ...
        let input_valueanticipo = parseFloat($("#anticipo").val());
        let montototal = parseFloat($('#ttl_gral_bs').val().replace('.', '').replace(',', '.'));
        switch (true) {
        case (anticipo == 0):
            $("#anticipo").val(0);
            break;
        case (montototal > anticipo):
            if (input_valueanticipo > anticipo) {
                ...
                Toast.fire({
                    icon: 'warning',
                    title: 'No puede exceder el monto Anticipo disponible'
                });
            }
            break;
        case (anticipo > montototal):
            if (input_valueanticipo > montototal) {
                $("#anticipo").val(montototal);
                const Toast = Swal.mixin({
                    toast: true,
                    position: 'top-end',
                    showConfirmButton: false,
                ...
                Toast.fire({
                    icon: 'warning',
                    title: 'No puede exceder el monto del documento'
                });
            }
            break;
        }
...
function cantPrdResult(search = "") {
    ...
    $.ajax({
        ...
        url: "ventas2_fac_data.php?op=buscar_cant_prd",
...
function actualizarExistenciasEnCasoError() {
    ...
    $.ajax({
        url: "ventas2_fac_data.php?op=existencias",
        type: "POST",
        data: formData,
        dataType: "json",
        contentType: false,
...
function mostrarTotales() {
    ...
    if (clie !== '' && vend !== '' && depo !== '') {
        ...
        let anticipo = Number($("#ant_input").val());
        $('#span_anticipo').text('(disponible ' + anticipo.format_money(2, 3, '.', ',') + ' Bs)');
        $("#anticipo").attr({
...
function listarProductos(search = "") {
    ...
    tabla_productos = $('#productos_data').dataTable({
        ...
        "ajax":
        {
            url: 'ventas2_fac_data.php?op=listar_prd',
            type: "POST",
            dataType: "json",
            data: {
                search: search,
...
function listarDocumentos(search = "") {
    ...
    tabla_documentos = $('#documentos_data').dataTable({
        ...
        "ajax":
        {
            url: 'ventas2_fac_data.php?op=listar_docs',
            type: "POST",
            dataType: "json",
...
function cantidadPrd(it) {
    ...
    if ((tipofac_c === '') || (tipofac_c !== '' && !tipofac_c.includes('C'))) {
        // si se ha cargado un pedido, no descuenta la unidad comprometida
        if (tipofac_c === 'E') {
            switch (true) {
                //si unid_selected = 0 ==> CAJ o BUL
                //si unid_selected = 1 ==> UND o BOT
            case ((unid_selected == 0 && (umb) < 1) || (unid_selected == 1 && ((((umb) > 0) ? (((umb) * uniemp) + (ump)) : (ump)) < 1))):
                ...
                if (!carga_de_documento_en_proceso) {
                    Swal.fire({
                        title: 'Atención!',
                        html: 'la cantidad de ' + tipo_unid_text + ' del producto ' + descrip + ' es inferior a 1',
                        icon: 'error',
                        allowOutsideClick: false
                    });
                }
            ...
            case (unid_selected == 1 && cant_selected > uniemp):
                ...
                break;
            case ((unid_selected == 0 && cant_selected > (umb)) || (unid_selected == 1 && (cant_selected > (((umb) > 0) ? (((umb) * uniemp) + (ump)) : (ump))))):
                $("#cant" + index).val((unid_selected == 0) ? (umb) : (((umb) > 0) ? (((umb) * uniemp) + (ump)) : (ump)));
                let descrip1 = $("#descrip" + index).val();
                let tipo_unid_text1 = (unid_selected == 1) ? $("#unid" + index + " option[value='1']").text() : $("#unid" + index + " option[value='0']").text();
                if (!carga_de_documento_en_proceso) {
                    Swal.fire({
                        title: 'Atención!',
                        html: 'la cantidad ingresada de ' + descrip1 + ' es mayor a la disponible (' + ((unid_selected == 0) ? (umb) : (((umb) > 0) ? (((umb) * uniemp) + (ump)) : (ump))) + ' ' + tipo_unid_text1 + ')',
                        icon: 'error',
                        allowOutsideClick: false
                    });
                }
                calculos_facturar();
                break;
            case (cant_selected >= ui && ((unid_selected == 0 && cant_selected <= (umb)) || (unid_selected == 1 && (cant_selected < (((umb) > 0) ? (((umb) * uniemp) + (ump)) : (ump)))))):
        ...
        // si no se ha cargado un pedido, entonces toma la existencia total no comprometida
        else {
            switch (true) {
                //si unid_selected = 0 ==> CAJ o BUL
                //si unid_selected = 1 ==> UND o BOT
...
function seleccionarPrd(codprod, idx = "") {
    ...
    if (renglones <= cant_renglones) {
        ...
        $.ajax({
            async: false,
            cache: false,
            url: "ventas2_fac_data.php?op=datos_prod",
            method: "POST",
            dataType: "json",
            data: {
                codprod: codprod,
                    ...
                    if (data.umb >= 1 || data.ump >= 1) {
                        ...
                        if (tipofac_c === 'E') {
                            switch (true) {
                                //si unid_selected = 0 ==> CAJ o BUL
                                //si unid_selected = 1 ==> UND o BOT
                            case (unid_selected == 0 && (data.umb) < 1) || (unid_selected == 1 && (((data.umb) > 0) ? (((data.umb) * uniemp) + (data.ump)) : (data.ump)) < 1):
                                $("#unid" + index).val((unid_selected == 1) ? 0 : 1).trigger('click');
                                $("#cant" + index).val(1);
                                $('#item' + index).trigger(jQuery.Event('keypress', { keyCode: 13 }));
                                ...
                                Swal.fire({
                                    title: 'Atención!',
                                    html: 'la cantidad de ' + tipo_unid_text + ' del producto ' + data.descrip + ' es inferior a 1',
                                    icon: 'error',
                                    allowOutsideClick: false
                                });
                                break;
                            ...
                            case (unid_selected == 1 && cant_selected > data.cantempaq):
                                ...
                                break;
                            case ((unid_selected == 0 && cant_selected > (data.umb)) || (unid_selected == 1 && (cant_selected > (((data.umb) > 0) ? (((data.umb) * uniemp) + (data.ump)) : (data.ump))))):
                                $("#cant" + index).val(parseInt((unid_selected == 0) ? (data.umb) : (((data.umb) > 0) ? (((data.umb) * uniemp) + (data.ump)) : (data.ump))));
                                let tipo_unid_text1 = (unid_selected == 1) ? $("#unid" + index + " option[value='1']").text() : $("#unid" + index + " option[value='0']").text();
                                Swal.fire({
                                    title: 'Atención!',
                                    html: 'la cantidad ingresada de ' + data.descrip + ' es mayor a la disponible (' + ((unid_selected == 0) ? (data.umb) : (((data.umb) > 0) ? (((data.umb) * uniemp) + (data.ump)) : (data.ump))) + ' ' + tipo_unid_text1 + ')',
                                    icon: 'error',
                                    allowOutsideClick: false
                                });
                                break;
...
function seleccionarDoc(numerod, tipofac) {
    $.ajax({
        async: false,
        cache: false,
        url: "ventas2_fac_data.php?op=datos_doc",
        method: "POST",
        dataType: "json",
        data: {
            numerod: numerod,
...
function calculos_facturar() {
    ...
    $("#form_table").serializeArray().forEach(value => {
        ...
        if (value.name === 'exento[]' && value.value !== '') {
            exento = value.value;
        }
        ...
        if (value.name === 'subtotal[]' && value.value !== '0') {
            switch (exento) {
            case '0': tgrabable += parseFloat(value.value); break;
            case '1': texento += parseFloat(value.value); break;
            }
            subtotal += parseFloat(value.value);
        }
    });

    total = ( (Math.round(texento*100)/100) + tgrabable + iva16 + ivaper + ial);

    // variables oculta
    $('#subttl_input').val(subtotal);
    $('#tgrabable_input').val(tgrabable);
    $('#texento_input').val(texento);
    $('#imp_16_input').val(iva16);
    $('#imp_per_input').val(ivaper);
    $('#imp_18_input').val(ial);
    $('#ttlbs_input').val(total);

    //variables visibles
    $('#subttl').text(((Math.round(texento * 100) / 100) + (Math.round(tgrabable * 100) / 100)).format_money(2, 3, '.', ','));
    $('#imp_16').text((Math.round(iva16 * 100) / 100).format_money(2, 3, '.', ','));
    $('#imp_per').text((Math.round(ivaper * 100) / 100).format_money(2, 3, '.', ','));
    $('#imp_18').text((Math.round(ial * 100) / 100).format_money(2, 3, '.', ','));
...
function calculos_totales() {
    ...
    let tgrabable = parseFloat($('#tgrabable_input').val());
    let texento = parseFloat($('#texento_input').val());
    let subttl = ((Math.round(texento*100)/100) + tgrabable);
...
function setFechaV(tipo = '') {
    ...
    let ant = $('#ant').val();
    let total_anticipo = (ant !== '') ? ant : 0;
    let input_anticipo = parseFloat($("#anticipo").val());
    let input_totalgral = parseFloat($("#ttl_gral_bs").val().replace('.', '').replace(',', '.'));
    if (input_anticipo > input_totalgral) {
        Swal.fire({
            title: 'Atención!',
            html: 'Anticipo ingresado es Superior al Monto del documento.',
            icon: 'error',
            allowOutsideClick: false
        });
        return (false);
    ...
    $.ajax({
        url: "ventas2_fac_data.php?op=facturar",
        type: "POST",
        data: formData,
        dataType: "json",
        contentType: false,
    ...
    if (icono.includes('error')) {
        ...
        arr_notas = Array();
                    //setTimeout(() => {  window.open('ventas2_fac_pdf1.php?&i=' + data.id, '_blank'); }, 2000);
    }
}
});
}
