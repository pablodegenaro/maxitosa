...
$consul_des_det = mssql_query("SELECT * from appfacturas_det where correl = '$correl'");
...
$genera = mssql_query("
	...
	TipoFac in ('A','C') AND (numerod in ($condiciones)) order by SAITEMFAC.CodItem
	");
...
$pdf->Ln();
$fact_new = mssql_query("SELECT *, MtoTotal/factorp as montod from safact where (numerod in ($condiciones)) and tipofac in ('A','C') order by numerod");
$num_fact_new = mssql_num_rows($fact_new);
for($m=0;$m<$num_fact_new;$m++){
	$fact_clie = mssql_query("SELECT DISTINCT(codclie) from safact where (numerod in ($condiciones)) and tipofac in ('A','C') ");
