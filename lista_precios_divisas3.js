...
function obtener_insta_hijos() {
    ...
    if (instap.length > 0) {
        $.ajax({
            url: "lista_precios_divisas3_data.php?op=instancias_hijo",
            type: "POST",
            dataType: "json",
            data: { instap: instap },
            beforeSend: function () {
...
function generarPdf() {
    ...
    if (depo !== "" && marca.length > 0 && instap.length > 0 && insta.length > 0 && prove.length > 0 && orden !== "") {
        ...
        window.open('lista_precios_divisas3_pdf.php?&' + datos, '_blank');
    } else {
        Swal.fire({
            title: "Atención!",
...
function generarExcel() {
    ...
    if (depo !== "" && marca.length > 0 && instap.length > 0 && insta.length > 0 && prove.length > 0 && orden !== "") {
        let datos = $('#formulario').serialize();
        window.open('lista_precios_divisas3_excel.php?&' + datos, '_blank');
    } else {
        Swal.fire({
            title: "Atención!",
            html: "Hay campos faltantes",
