...
switch ($_GET["op"]) {
    ...
    $sacorrel =  mssql_query("SELECT Prefijo, ValueInt FROM SACORRELSIS WHERE FieldName='PrxFact' AND CodSucu='$codsucu'");
    ...
    $datos =  Mssql::fetch_assoc(
        mssql_query("SELECT p.CodProd, p.Descrip, CONVERT(INT, p.CantEmpaq) CantEmpaq, CONVERT(INT, e.Existen) Cajas, CONVERT(INT, e.ExUnidad) Botellas, CONVERT(INT, e.CantCom) CajasCompr, CONVERT(INT, e.UnidCom) BotellasCompr,
            p.Unidad, p.UndEmpaq, p.EsExento, p.Precio$precio/(CASE WHEN  '$unid' = '1' THEN p.CantEmpaq ELSE 1 END) AS Precio, 
    ...
    switch (true) {
        ...
        $datos = Mssql::fetch_assoc(
            mssql_query("SELECT NumeroD, Descrip, TipoFac, MtoTotal, FechaE, ISNULL(MtoTotal/FactorP, 0) Mtotald 
                FROM SAFACT WHERE TipoFac IN ('C','E','F') AND CodSucu='$sucursal' AND (NumeroD LIKE '%$search%' OR Descrip LIKE '%$search%')")
        );
        break;
        case ($search=='' && $codclie==''):
        $datos = Mssql::fetch_assoc(
            mssql_query("SELECT NumeroD, Descrip, TipoFac, MtoTotal, FechaE, ISNULL(MtoTotal/FactorP, 0) Mtotald 
                FROM SAFACT WHERE TipoFac IN ('C','E','F') AND CodSucu='$sucursal'")
        );
        break;
        case ($search!=='' && $codclie!==''):
        $datos = Mssql::fetch_assoc(
        ...
        $datos = Mssql::fetch_assoc(
            mssql_query("SELECT NumeroD, Descrip, TipoFac, MtoTotal, FechaE, ISNULL(MtoTotal/FactorP, 0) Mtotald 
                FROM SAFACT WHERE TipoFac IN ('C','E','F') AND CodSucu='$sucursal' AND CodClie='$codclie'")
        );
        break;
    }
    ...
    $saitemfac =  Mssql::fetch_assoc(
        mssql_query("SELECT CodItem, Descrip1, item.CodUbic, Cantidad, item.EsUnid, p.UndEmpaq, CONVERT(INT, p.CantEmpaq) CantEmpaq, Precio, Precio/item.FACTORP AS Preciod, item.FactorP, TipoPVP, Descto, Descto/item.FactorP AS Desctod,
            CONVERT(INT, e.Existen) Cajas, CONVERT(INT, e.ExUnidad) Botellas, CONVERT(INT, e.CantCom) CajasCompr, CONVERT(INT, e.UnidCom) BotellasCompr, p.EsExento,
        ...
        if ($_SESSION['codsucu']) 
        {
            ...
            if ($_SESSION['codesta']) 
            {
                ...
                foreach ($arr_prod as $i => $codprod) {
                    if ($codprod!='') {
                        ...
                        if (mssql_num_rows($query) > 0) {
                                // si el documento a generar es tipofac='A' y tipofac_c='C', significa que proviene de una
                                // nota de entrega que va a pasar a factura, por lo cual no se considera la existencia disponible
                                // ya que al generar la nota de entrega se desconto del inventario
                            if ($tipofac_c!='' && $tipofac_c!='C') 
                            {
                ...
                if ($flag_exis) {
                    ...
                    $querylengh = mssql_query("SELECT ValueInt FROM SACORRELSIS WHERE FieldName='LenCorrel' AND CodSucu='$codsucu'");
                    $lengh = (mssql_num_rows($querylengh)>0) ? mssql_result($querylengh, 0,"ValueInt") : 8;
                    $query = mssql_query("SELECT FieldName, Prefijo, ValueInt FROM SACORRELSIS WHERE FieldName='PrxFact' AND CodSucu='$codsucu'");
                    ...
                    if ($correl_nuevo != "") {
                        ...
                        $query = mssql_query("UPDATE SACORRELSIS SET ValueInt=ValueInt+1 WHERE FieldName='PrxFact' AND CodSucu='$codsucu'");
                    ...
                    if($procesar){
                        ...
                        $output = array(
                            ...
                            "title" => 'Completado',
                            "mensaje" => "Se Facturó exitosamente!",
                            "icono"   => "success"
                        );
                    } else {
                        $output = array(
                            "id"   => "",
                            "title" => 'Atención!',
                            "mensaje" => "Ocurrió un error al Facturar! ".$mensaje_err,
                            "icono"   => "error"
                        );
                    }
                    ...
                    if (!$procesar || !$flag_items) {
                        ...
                        $mail->Subject = "ERROR APP EN FACTURACION";
