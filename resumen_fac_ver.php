...
if ($_SESSION['login']) {
  $fechai = $_POST['fechai'];
// $fechai = normalize_date2($fechai);a
  $fechaf = $_POST['fechaf'];
...
//$convend = $_POST['edv'];
  ...
  switch (true) {
    ...
    $query = mssql_query("SELECT a.fechae, a.NumeroD, a.TipoFac,  a.CodClie, a.Descrip, b.clase, a.MtoTotal, a.factorp, a.MtoTotal/a.FactorP as totald, a.codvend,
      (select ISNULL(sum(cantidad),0) from saitemfac where saitemfac.numerod = A.numerod and saitemfac.tipofac ='A' and EsUnid = '0') as cajas,
      (select ISNULL(sum(cantidad),0) from saitemfac where saitemfac.numerod = A.numerod and saitemfac.tipofac ='A' and EsUnid = '1') as unidades
      ...
      where a.TipoFac in ('A') and  a.numeror is null and   DATEADD(dd, 0, DATEDIFF(dd, 0, a.FechaE)) between '$fechai' and '$fechaf' and a.CodSucu = '$sucursal'   group by a.numerod, a.fechae, a.TipoFac, a.CodClie, a.Descrip, b.Clase, a.MtoTotal, a.FactorP,a.CodVend order by fechae desc");
    break;
    ...
    $query = mssql_query("SELECT a.fechae, a.NumeroD, a.TipoFac,  a.CodClie, a.Descrip, b.clase, a.MtoTotal, a.factorp, a.MtoTotal/a.FactorP as totald, a.codvend,
      ...
      (select ISNULL(sum(cantidad),0) from saitemfac where saitemfac.numerod = A.numerod and saitemfac.tipofac ='A' and EsUnid = '1') as unidades
      from SAFACT as a left join SACLIE as b on a.CodClie=b.CodClie inner join saitemfac as c on a.numerod=c.numerod and a.tipofac=c.tipofac 
      where a.TipoFac in ('A') and  a.numeror is null and    DATEADD(dd, 0, DATEDIFF(dd, 0, a.FechaE)) between '$fechai' and '$fechaf'  group by a.numerod, a.fechae, a.TipoFac, a.CodClie, a.Descrip, b.Clase, a.MtoTotal, a.FactorP,a.CodVend order by fechae desc");
    break;
      ...
      <div class="card card-saint">
        <div class="card-header">
          <script type="text/javascript">
            function regresa(){
              window.location.href = "principal1.php?page=resumen_fac&mod=1";
            }
          </script>
                ...
                <?php for ($i = 0; $i < mssql_num_rows($query); $i++) {
                  ...
                  <tr>
                    ...
                    <label for="tipo_fact" class="col-form-label-sm" >
                      <?php 
                      if (mssql_result($query,$i,"tipofac") == 'A') {
                        echo "Factura";
                      } else { echo "Devolucion FAC";} ?></label>
                    </div>
                  </td>
...
<script type="text/javascript">
  ...
  function ver_fact(numerod= -1, tipofac, codclie, descrip) {
    ...
    if(numerod !== -1)
    {
      let tipo = (tipofac==='A') ? " (Factura)" : " (Devolucion Fac)";
