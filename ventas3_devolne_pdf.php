...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$querySafact = mssql_query("SELECT a.numerod, a.tipofac, a.codclie, a.ID3, a.Descrip, a.Direc1, A.Direc2, a.Telef, c.Descrip vendedor, ISNULL(a.Descto1/a.FactorP,0) Descto1, NumeroR,
			c.codvend, ISNULL(a.MtoTotal/a.FactorP,0) MtoTotal, a.fechae, a.fechav, a.notas1, a.Descto1, ISNULL(a.Monto/a.FactorP,0) Monto, z.Descrip zona, EsCredito, ISNULL((Monto+Mtotax)/a.FactorP,0) SubTotal,
		...
		$this->Cell(112,6, "NOTA DE ENTREGA A NOMBRE DE:",0,0,'L');
		$this->Cell(42,6, utf8_decode("DEV NE N°:"),0,0,'R');
		...
		$this->SetFont('Arial','B',9);
		$this->Cell(20,4, mssql_result($querySafact,0,"DiasCredito"),0,1,'L');$this->Cell(129);
		$this->SetFont('Arial','',8);
		$this->Cell(25,4, "N/E Afectada: ",0,0,'L');
		$this->SetFont('Arial','B',9);
		$this->Cell(20,4, mssql_result($querySafact,0,"NumeroR"),0,1,'L');
...
$pdf->SetY(-22);
$pdf->SetFont('Arial','',8);
$pdf->Cell(145); $pdf->Cell(25,4,'Subtotal $','',0,'L',0);
$pdf->SetFont('Arial','',10);
$pdf->Cell(24,4, number_format($monto+$montoImp, 2, ',', '.').'  USD','',1,'R',0);

$pdf->SetY(-18);
$pdf->SetFont('Arial','',8);
$pdf->Cell(145); $pdf->Cell(25,4,'Descto $','',0,'L',0);
$pdf->SetFont('Arial','',10);
$pdf->Cell(24,4, number_format($descto, 2, ',', '.').'  USD','',1,'R',0);

$pdf->SetY(-14);
$pdf->SetFont('Arial','',8);
$pdf->Cell(145); $pdf->Cell(25,4,'Total $','',0,'L',0);
$pdf->SetFont('Arial','',10);
$pdf->Cell(24,4, number_format($total, 2, ',', '.').'  USD','',1,'R',0);
