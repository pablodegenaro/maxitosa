	...
	<div class="content-wrapper">
		<div class="content-header">
			<div class="container">
				<div class="row mb-2">
					<div class="col-sm-6">
						<h1 class="m-0 text-dark">Libro de Ventas CARUPANO</h1>
					</div>
				</div>
				<div class="row mb-2">
					...
					<div class="col-sm-3 mt-4 form-check-inline">
						...
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<script type="text/javascript">
							function regresa(){
								window.location.href = "principal1.php?page=libro_ventas_carupano&mod=1";
							}
						</script>
		...
		<div class="content">
			<?php     $resumen1 = mssql_query("SET DATEFORMAT YMD; SELECT * FROM DBO.VW_ADM_LIBROIVAVENTAS_CARUPANO WHERE CODSUCU = '$sucursal' and  ('$fechai'<=FECHAEMISION) AND (FECHAEMISION<='$fechaf') AND ((FECHARETENCION IS NULL) OR (('$fechai'<=FECHARETENCION) AND (FECHARETENCION<='$fechaf'))) and TIPODOC != 'RET' ORDER BY (YEAR(FECHAFACTURA)*10000)+(MONTH(FECHAFACTURA)*100)+DAY(FECHAFACTURA),FECHAT");
			$resumen2 = mssql_query("SET DATEFORMAT YMD; SELECT * FROM DBO.VW_ADM_LIBROIVAVENTAS_CARUPANO WHERE CODSUCU = '$sucursal' and  ('$fechai'<=FECHAEMISION) AND (FECHAEMISION<='$fechaf') AND (NOT(FECHARETENCION IS NULL) AND NOT(('$fechai'<=FECHARETENCION) AND (FECHARETENCION<='$fechaf'))) AND TIPO='81' ORDER BY FECHAT");
		...
		<div id="Layer2" style="width:1400px; height:auto;">
			...
			<div align="center">
				<a href="libro_ventas_carupano_excel.php?&fechai=<?php echo $fechaii; ?>&fechaf=<?php echo $fechaff; ?>&sucu=<?php echo $sucursal; ?>"><img src="images/excel.jpeg" width="19" height="18" border="0" /> Exportar a Excel</a>
			</div>
			<br>
		</div>
