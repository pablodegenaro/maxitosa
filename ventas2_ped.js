...
function correlYtasa() {
    $.post("ventas2_ped_data.php?op=index", function (data, status) {
...
$(document).ready(function () {
    $("#clie").change(() => {
        if ($("#clie").val() !== "") {
            ...
            $.post("ventas2_ped_data.php?op=datos_clie", { codclie: codclie }, function (data, status) {
                if (!jQuery.isEmptyObject(data)) {
...
function cantPrdResult(search = "") {
    ...
    $.ajax({
        ...
        url: "ventas2_ped_data.php?op=buscar_cant_prd",
...
function actualizarExistenciasEnCasoError() {
    ...
    $.ajax({
        url: "ventas2_ped_data.php?op=existencias",
        type: "POST",
        data: formData,
        dataType: "json",
        contentType: false,
...
function listarProductos(search = "") {
    ...
    tabla_productos = $('#productos_data').dataTable({
        ...
        "ajax":
        {
            url: 'ventas2_ped_data.php?op=listar_prd',
            type: "POST",
            dataType: "json",
            data: {
                search: search,
...
function listarDocumentos(search = "") {
    ...
    tabla_documentos = $('#documentos_data').dataTable({
        ...
        "ajax":
        {
            url: 'ventas2_ped_data.php?op=listar_docs',
            type: "POST",
            dataType: "json",
...
function seleccionarPrd(codprod, idx = "") {
    ...
    if (renglones <= cant_renglones) {
        ...
        $.ajax({
            async: false,
            cache: false,
            url: "ventas2_ped_data.php?op=datos_prod",
            method: "POST",
            dataType: "json",
            data: {
                codprod: codprod,
...
function seleccionarDoc(numerod, tipofac) {
    $.ajax({
        async: false,
        cache: false,
        url: "ventas2_ped_data.php?op=datos_doc",
        method: "POST",
        dataType: "json",
        data: {
            numerod: numerod,
...
function procesarPedido(e) {
    ...
    $.ajax({
        url: "ventas2_ped_data.php?op=generarpedido",
        type: "POST",
        data: formData,
        dataType: "json",
        contentType: false,
