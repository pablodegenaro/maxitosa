...
if (empty($_GET["correl"])) {
	echo "<script language=Javascript> location.href=\"principal1.php?page=despacho_creaft&mod=1\";</script>";
} else {
	$update = mssql_query("UPDATE appfacturasft set fechad='$fechai', nota ='$destino', placa ='$placa', cedula_chofer ='$chofer' where correl = '$correl'");
	if ($update){
...
// Sender &amp; Recipient
    	...
    	//$mail->AddCC("");
    	//$mail->AddBCC("");
		$mail->Subject = "Se Edito el Despacho # $correl FT";
		$body="El usuario $usuario Edito el Despacho # $correl . FT";
		$mail->Body=$body;
		$mail->Send();
